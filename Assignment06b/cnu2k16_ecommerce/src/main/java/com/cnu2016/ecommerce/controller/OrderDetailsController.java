...
public class OrderDetailsController {
    ...
    public ResponseEntity<?> addProductIntoOrder(@PathVariable Integer orderId, @RequestBody OrderProductPOJO body) {
        ...
        OrderDetails orderDetails = orderDetailsRepository.findByOrderIdAndProductId(orders.getOrderId(), product.getProductID());
        OrderDetails newOrderDetails;
        if (orderDetails == null) {
            newOrderDetails = new OrderDetails(orders.getOrderId(), product.getProductID(), body.getQty(),
                    product.getCostPrice(), product.getSellingPrice(), orders, product);
        } else {
            newOrderDetails = new OrderDetails(orderDetails.getOrderId(), orders.getOrderId(), product.getProductID(),
                    orderDetails.getQuantityOrdered() + body.getQty(),
